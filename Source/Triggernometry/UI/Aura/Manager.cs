...
internal class Manager : IDisposable {
	...
	// public void ManagerThread(IFramework _)
	// {
	//     int numTicks = 0;
		...
		try {
			...
			if (a.ctx != null && a.ctx.trig != null)
			{
			    messages.Add(new DeferredMessage()
			    {
			        ...
			        Message = I18n.Translate("internal/AuraContainer/updateerror", String.Format("Deactivating aura '{0}' from trigger '{1}' due to update exception: {2}", si.Key, si.Value.ctx.trig.LogName, ex.Message))
			    }
			    );
			}
			else
