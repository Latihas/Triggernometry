...
public class Graphics : IDisposable {
	...
	public void EndScene() {
		...
		long exa, exb;
		var result = _device.TryEndDraw(out exa, out exb);
	...
	public Scene UseScene() => new(this);
	...
	public Geometry CreateGeometry() => new(this);
	...
	public void DrawTextEx(Font font, FontStyle fs, float fontSize, IBrush brush, ParagraphAlignment pa, TextAlignment ta, float x, float y, float width, float height, string text) {
		if (!IsDrawing) throw ThrowHelper.UseBeginScene();
		...
		if ((fs & FontStyle.Underline) == FontStyle.Underline) {
			layout.SetUnderline(true, new TextRange(0, text.Length));
		}
		if ((fs & FontStyle.Strikeout) == FontStyle.Strikeout) {
			layout.SetStrikethrough(true, new TextRange(0, text.Length));
		}
		layout.ParagraphAlignment = pa;
		layout.TextAlignment = ta;
	...
	public void DrawImage(Image image, float x, float y, float opacity = 1.0f) {
		...
		var destRight = x + image.Bitmap.PixelSize.Width;
		var destBottom = y + image.Bitmap.PixelSize.Height;
	...
	public void DrawImageEx(Image image, Rectangle srcRectangle, Rectangle dstRectangle, float opacity = 1.0f, bool linearScale = true) {
		if (!IsDrawing) throw ThrowHelper.UseBeginScene();
		...
		_device.DrawBitmap(
			...
			opacity,
			linearScale ? BitmapInterpolationMode.Linear : BitmapInterpolationMode.NearestNeighbor,
			new RawRectangleF(srcRectangle.Left, srcRectangle.Top, srcRectangle.Left + srcRectangle.Width, srcRectangle.Top + srcRectangle.Height)
		);
	}
	...
	///     <see langword="false" />.
	/// </returns>
	public static bool Equals(Graphics left, Graphics right) => left?.Equals(right) == true;
}