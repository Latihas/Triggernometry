...
internal static class ImagePixelFormats {
	...
	private static readonly Guid[] _bestPixelFormats = [
	...
	private static readonly Guid[] _mostCommonPixelFormats = [
		PixelFormat.Format32bppRGBA,
		PixelFormat.Format32bppBGRA,
		PixelFormat.Format32bppPRGBA,
		PixelFormat.Format32bppPBGRA,
	...
	public static Guid GetBestPixelFormat(Guid[] supportedFormats) {
		...
		for (var i = 0; i < _mostCommonPixelFormats.Length; i++) {
			for (var x = 0; x < supportedFormats.Length; x++) {
				if (_mostCommonPixelFormats[i] == supportedFormats[x]) return supportedFormats[x];
			}
		}

		for (var i = 0; i < _bestPixelFormats.Length; i++) {
			for (var x = 0; x < supportedFormats.Length; x++) {
