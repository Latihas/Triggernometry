...
public readonly struct IndexMemberExpression {
	...
	/// </summary>
	public string[] Indexes => _indexes ?? [];
	...
	public IndexMemberExpression(string expr) {
		...
		var rawPossibleName = ScanPossibleName(expr, out var lBracketPos, out var validDotPos);
		...
		if (lBracketPos >= 0) {
			var indexList = ScanIndexList(expr, lBracketPos + 1, out validDotPos);
	...
	private static List<string> ScanIndexList(
		...
		for (var i = startIndex; i < expr.Length; i++) {
			var c = expr[i];
			if (c != ']') continue;
			...
			var isValidClose = k >= expr.Length || expr[k] == '[' || expr[k] == '.';
			if (!isValidClose) {
