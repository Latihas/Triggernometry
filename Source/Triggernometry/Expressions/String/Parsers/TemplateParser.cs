...
internal static class TemplateParser {
	...
	internal static List<TemplateMatch> FindNonNestedTemplates(string expr) {
		...
		for (var i = 0; i < fullLength - 1; i++) {
			...
			var validRightBracePos = -1;
			for (var j = i + 2; j < fullLength; j++) {
				// found valid close '}'
				if (expr[j] == '}') {
				...
				var notInnerDollar = j >= fullLength - 1 || expr[j] != '$';
	...
	private static void AddBraceTemplate(List<TemplateMatch> templateMatches, string fullExpr, int dollarPos, int rBracePos, out int newScanPos) {
		...
		var innerExpr = fullExpr.Substring(dollarPos + 2, length); // without "${" / "}"
	...
	internal static string ReplaceCurrencyTemplates(string expr) {
		...
		for (var i = 0; i < n; i++) {
			...
			var currencyCount = CheckConsecutiveCurrencyCharCount(expr, i, out var isValidCurrencyTemplate);
			if (currencyCount > 0) {
				var c = isValidCurrencyTemplate ? '$' : '¤';
