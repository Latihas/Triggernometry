...
internal static class ColonParser {
    internal static string TryParse(string template, Context ctx) {
        ...
        switch (prefixLower) {
            ...
            case "?lvar":
            case "?l": {
                var evaluator = ListEvaluator.BuildEvaluator(expr);
                ResolveVarAccess(prefixLower, ctx, out var store, out var mustExist, out var isTemp);
                if (isTemp) {
                    var vl = VariableList.BuildTemp(expr.Name); // name is actually expression: "1, 2, 3"
                    return evaluator(vl);
                }
                lock (store.List) {
                    var vl = GetVariableWithCondition(store, store.List, expr.Name, mustExist);
                    return evaluator(vl);
                }
            }
            ...
            case "?dvar":
            case "?d": {
                var evaluator = DictEvaluator.BuildEvaluator(expr);
                ResolveVarAccess(prefixLower, ctx, out var store, out var mustExist, out var isTemp);
                if (isTemp) {
                ...
                lock (store.Dict) {
                    var vd = GetVariableWithCondition(store, store.Dict, expr.Name, mustExist);
                    return evaluator(vd);
                }
            }
            ...
            case "?tvar":
            case "?t": {
                var evaluator = TableEvaluator.BuildEvaluator(expr);
                ResolveVarAccess(prefixLower, ctx, out var store, out var mustExist, out var isTemp);
                if (isTemp) {
                ...
                lock (store.Table) {
                    var vt = GetVariableWithCondition(store, store.Table, expr.Name, mustExist);
                    return evaluator(vt);
                }
            }
            ...
            case "!ptrl": {
                ...
                var headerExpr = expr.Index1;
                var idx = expr.Index2.Equals("last", StringComparison.OrdinalIgnoreCase) ? -1 : (int)MathParser.Parse(expr.Index2);

                ResolveVarAccess(prefixLower, ctx, out var store, out var mustExist, out _);
                lock (store.Table) {
                    var vt = GetVariableWithCondition(store, store.Table, expr.Name, mustExist);
                    var rowIdxFrom1 = vt.SeekRow(headerExpr);
                    if (rowIdxFrom1 > 0) {
                        var colIdxFrom1 = idx + (idx >= 0 ? 1 : 0);
            ...
            case "!ptcl": {
                ...
                lock (store.Table) {
                    var vt = GetVariableWithCondition(store, store.Table, expr.Name, mustExist);
                    var colIdxFrom1 = vt.SeekColumn(headerExpr);
                    if (colIdxFrom1 > 0) {
                        var rowIdxFrom1 = idx + (idx >= 0 ? 1 : 0);
                        return vt.Peek(colIdxFrom1, rowIdxFrom1).ToString();
                    }
                }
                // not found:
            ...
            case "!ptdl": {
                ...
                var colHeader = expr.Index1;
                var rowHeader = expr.Index2;
                ResolveVarAccess(prefixLower, ctx, out var store, out var mustExist, out _);
                lock (store.Table) {
                    var vt = GetVariableWithCondition(store, store.Table, expr.Name, mustExist);
                    // index starts from 1; 0 if not found
                    var colIdxFrom1 = vt.SeekColumn(colHeader);
                    var rowIdxFrom1 = vt.SeekRow(rowHeader);
