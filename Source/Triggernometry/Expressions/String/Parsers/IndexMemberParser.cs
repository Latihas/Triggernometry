	...
	internal static string TryParse(string template, Context ctx) {
		...
		switch (expr.Name) {
			...
			case "_jsonresponse":
				...
				var jsonPath = expr.Index;
				var node = ctx.contextJsonResponse;
				foreach (var nodeName in jsonPath.Split("/".ToCharArray())) {
					node = node[nodeName];
				}
			...
			case "_textaura": {
				if (plug.sc != null) {
					lock (plug.sc.textitems) {
						var item = plug.sc.GetText(expr.Index);
						if (item == null) return "";
					...
					lock (plug.textauras) {
						if (!plug.textauras.TryGetValue(expr.Index, out var acf))
							return "";
			...
			case "_imageaura": {
				if (plug.sc != null) {
					lock (plug.sc.imageitems) {
						var item = plug.sc.GetImage(expr.Index);
						if (item == null) return "";
						switch (expr.Member.Name.ToLowerInvariant()) {
							case "x":
								return I18n.ThingToString(item.Left);
