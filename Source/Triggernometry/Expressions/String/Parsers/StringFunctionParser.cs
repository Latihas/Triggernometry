	...
	internal static string TryParse(string operand) {
		...
		switch (funcNameLower) {
			...
			case "float2hex":
				CheckArgCountLocal("0");
			{
				if (!float.TryParse(sourceString, NSFloat, InvClt, out var floatValue)) {
					throw ParseTypeError(I18n.TranslateWord("string"), sourceString, I18n.TranslateWord("float"));
				}
				var bytesArray = BitConverter.GetBytes(floatValue);
			...
			case "double2hex":
				CheckArgCountLocal("0");
			{
				if (!double.TryParse(sourceString, NSFloat, InvClt, out var doubleValue)) {
					throw ParseTypeError(I18n.TranslateWord("string"), sourceString, I18n.TranslateWord("double"));
				}
				var bytesArray = BitConverter.DoubleToInt64Bits(doubleValue);
			...
			case "ord": // chars => charcodes separated by separator
				CheckArgCountLocal("0-1");
			{
				var separator = GetArgument(args, 0, ",");
				var charcodes = new List<int>();
				for (var idx = 0; idx < sourceString.Length; idx++) {
			...
			{
				var separator = GetArgument(args, 0, ",");
				var rawCharcodes = SplitArguments(sourceString, separator: separator);
				var chars = new List<string>();
				for (var idx = 0; idx < rawCharcodes.Length; idx++) {
					if (int.TryParse(rawCharcodes[idx], out var charcode)) {
			...
			{
				var paddingChar = args[0].Length == 1 ? args[0][0] : GetReplacedChar(int.Parse(args[0], InvClt));
				var length = int.Parse(args[1], InvClt);
			...
			{
				if (!int.TryParse(args[0], NSFloat, InvClt, out var times)) {
				...
				var repeatedUnit = joiner + sourceString;
				for (var repeatCount = 1; repeatCount < times; repeatCount++) {
				...
				CheckArgCountLocal("1-3");
			{
				...
				var newStr = GetArgument(args, 1, "");
				if (newStr == oldStr) {
				}

				var isLoopedStr = GetArgument(args, 2, "false");
				if (!bool.TryParse(isLoopedStr, out var isLooped)) {
			...
			case "dictreplace": // dictreplace(old1 = new1, old2 = new2, ...)
			{
				...
				foreach (var pair in args) {
					var kv = SplitArguments(pair, separator: "=");
			...
			case "substring": // substring(startindex, length) or substring(startindex)
				CheckArgCountLocal("1-2");
			{
				if (!int.TryParse(args[0], NSFloat, InvClt, out var startIndex))
			...
			case "slice": // slice(slices = ":") 
				CheckArgCountLocal("0-1");
			{
				var slicesStr = GetArgument(args, 0, ":");
				var indices = GetSliceIndices(slicesStr, sourceString.Length, null, 0);
			...
			case "pick": // pick(index, splitter = ",")
				CheckArgCountLocal("1-2");
			{
				var separator = GetArgument(args, 1, ",");
				var strArray = SplitArguments(sourceString, separator: separator);
				if (!int.TryParse(args[0], NSFloat, InvClt, out var index))
					throw ParseTypeError(I18n.TranslateWord("index"), args[0], I18n.TranslateWord("int"));

				var normIndex = index < 0 ? index + strArray.Length : index;
			...
			{
				var index = funcNameLower.StartsWith("i") ? sourceString.IndexOf(args[0]) : sourceString.LastIndexOf(args[0]);
				...
				CheckArgCountLocal("1-3");
			{
				...
				var subLength = targetStr.Length;
				var totalLength = sourceString.Length;
				var joiner = GetArgument(args, 1, ",");
				var slicesStr = GetArgument(args, 2, ":");
				var indices = GetSliceIndices(slicesStr, totalLength - subLength + 1, null, 0);
				var sb = new StringBuilder();
			...
			case "compare": // compare(stringtocompare) or compare(stringtocompare, ignorecase)
				CheckArgCountLocal("1-2");
				var ignoreCaseStr = GetArgument(args, 1, "true");
				if (!bool.TryParse(ignoreCaseStr, out var ignoreCase)) {
			...
			case "versioncompare": // ${f:versioncompare(1.2.0.0):1.1.8.0} = -1
				CheckArgCountLocal("1");
				var srcVersion = Version.TryParse(sourceString, out var v)
					? v
					: throw ParseTypeError(I18n.TranslateWord("string"), sourceString, I18n.TranslateWord("version"));
				var tgtVersion = Version.TryParse(args[0], out v)
					? v
					: throw ParseTypeError(I18n.TranslateWord("string"), args[0], I18n.TranslateWord("version"));
			...
			case "match": // func:match(str):regex
				CheckArgCountLocal("1");
			{
				var match = new Regex(UnescapeCustomExpr(sourceString)).Match(args[0]);
			...
			{
				var match = new Regex(UnescapeCustomExpr(sourceString)).Match(args[0]);
				if (int.TryParse(args[1], NSFloat, InvClt, out var groupNumber)
				...
				if (args.Length > 0) {
					foreach (var arg in args) {
						...
						if (arg.Length == 1) {
							...
							if (!int.TryParse(arg, NSFloat, InvClt, out var charcode)) {
				...
				CheckArgCountLocal("2");
			{
				...
				var converted = Convert.ChangeType(sourceString, type, InvClt);
				...
				CheckArgCountLocal("0-1");
			{
				...
				var dt = new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc).AddSeconds(ts);
				if (funcNameLower == "localtime") {
					dt = dt.ToLocalTime();
				}
				var format = GetArgument(args, 0, "");
