...
public static class StringParser {
	...
	///     · <paramref name="expr" />: Full input text that may contain templates such as <c>$n</c>, <c>${...}</c>,
	///     <c>¤{...}</c>.<br />
	///     · <paramref name="ctx" />: Evaluation context used during template evaluation; if null,
	///     <see cref="Context.Unbound" /> is used.<br />
	///     · <paramref name="isTestModeNumeric" />: Whether the expression is numeric. Only used when testing with
	///     placeholders.<br />
	/// </summary>
	internal static string Parse(string expr, Context ctx, bool isTestModeNumeric) {
		...
		// '\r' '\n' => '⏎'
		var newExpr = ReplaceLineBreak(expr);
