...
public static class ArgHelper {
	private static readonly object _lockValidators = new();

	private static readonly Dictionary<string, Func<int, bool>> _validators = new(StringComparer.OrdinalIgnoreCase);
	...
	private static Func<int, bool> GetArgCountValidator(string requiredCount) {
		...
		var checks = new List<Func<int, bool>>();
		int? last1 = null;
		int? last2 = null;
	...
	///     If the argument contains separator, single/double quotes, it should be quoted like "xx,xx", 'xx,xx', 'xx"xx',
	///     "xx'xx".
	/// </summary>
	...
	public static string[] SplitArguments(string args, bool allowEmptyList = true, string separator = ",") {
		...
		foreach (Match match in matches) {
			var currentMatch = match.Value.TrimEx();
	...
	///     · <paramref name="setEmptyToDefault" />: If the selected argument is empty, consider it as the default value or
	///     not.
	/// </summary>
	...
	///     The start index in the given expression. 0 for strings; 1 for other variables.
	///     (Triggernometry definition)
	/// </param>
	...
	public static List<int> GetSliceIndices(string slicesStr, int totalLength, string rawExpr, int startIndex) {
		...
		var slices = SplitArguments(slicesStr, false);
		foreach (var slice in slices) {
			// parse slice string to int start/end/step
			var sliceArgs = slice.Split(':').Select(s => s.TrimEx()).ToArray();
			CheckArgCount("0-3", sliceArgs.Length, I18n.TranslateWord("slice"));

			var startStr = GetArgument(sliceArgs, 0, "", true);
			var endStr = GetArgument(sliceArgs, 1, "", true);
			var stepStr = GetArgument(sliceArgs, 2, "1", true);
			var start = 0;
			var end = 0;
			var step = 0;
			try {
				...
				if (step == 0) {
					throw ErrorHelper.InvalidValueError(I18n.TranslateWord("slice"), "step", "0");
				}
				if (startStr != "") {
