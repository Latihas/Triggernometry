...
public static class DataStringHelper {
	...
	public static T ParseData<T>(this string input) => (T)input.ParseData(typeof(T));

	public static T ParseDataOrDefault<T>(this string input, T defaultValue) => TryParseData(input, typeof(T), out var result) ? (T)result : defaultValue;
