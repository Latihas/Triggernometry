...
public class MathParser {
	...
	static MathParser() {
		...
		AddOperator("√", true, 1, Math.Sqrt);
		AddOperator("!", true, 1, a => IsZero(a) ? 1 : 0); // logic not  (!!a: double => "bool")
		AddOperator("~", true, 1, a => ~Truncate(a)); // bitwise not
		...
		LocalFunctions.Add("or", OrFunction);
		LocalFunctions.Add("and", AndFunction);
		...
		LocalStringFunctions.Add("hex2dec", Hex2DecFunction);
		LocalStringFunctions.Add("hex2float", Hex2FloatFunction);
		LocalStringFunctions.Add("hex2double", Hex2DoubleFunction);
		LocalStringFunctions.Add("X8float", Hex2FloatFunction);
		LocalStringFunctions.Add("parsedmg", ParseDamage);
		LocalStringFunctions.Add("len", x => x[0].Length);
		LocalStringFunctions.Add("freq", Frequency);
		LocalStringFunctions.Add("nextETms", NextETms);
	...
	/// <summary>
	///     When comparing with an number n,
	///     all numbers in the range (n ± TOLERANCE) would all be considered as == n.
	///     double.Epsilon is too small for Triggernometry use.
	/// </summary>
	public const double TOLERANCE = 1E-9;
	public static bool IsZero(double x, double tolerance = TOLERANCE) => Math.Abs(x) < tolerance;
	...
	public static double ModFunction(double a, double b) => (a % b + b) % b;

	public static double IfFunction(double a, double b, double c) => IsZero(a) ? c : b;
	...
	public static double IsAngleBetweenFunction(double[] input) {
		switch (input.Length) {
			case 3:
				...
				var inRange = θ1 <= θ2 ? θ1 <= θ && θ <= θ2 : θ1 <= θ || θ <= θ2;
				return inRange ? 1 : 0;
			default:
				return 0;
		}
	...
	public static double Hex2DecFunction(string[] x) {
		var ex = long.Parse(x[0], NumberStyles.HexNumber, CultureInfo);
		double xy = ex;
		return xy;
	}

	public static double Hex2FloatFunction(string[] x) {
		var bytesArray = int.Parse(x[0], NumberStyles.HexNumber, CultureInfo);
		var f = BitConverter.ToSingle(BitConverter.GetBytes(bytesArray), 0);
		return f;
	}

	public static double Hex2DoubleFunction(string[] x) {
		var bytesArray = long.Parse(x[0], NumberStyles.HexNumber, CultureInfo);
		var d = BitConverter.ToDouble(BitConverter.GetBytes(bytesArray), 0);
		return d;
	}

	public static double ParseDamage(string[] x) => ParseDamage(x[0]);
	...
	public static double Frequency(string[] x) {
		...
		if (mx.Success) {
			var semitones = x.Length > 1 ? Parse(x[1]) : 0;
			var noteName = mx.Groups["note"].Value;
			switch (noteName) {
			...
			var signs = mx.Groups["signs"].Value;
			...
			var octaves = int.Parse(mx.Groups["octaves"].Value, CultureInfo);
	...
	public static List<string> OperatorOrder { get; set; } = [];
	...
	public static List<string> Lexer(string expr) {
		...
		for (var i = 0; i < expr.Length; i++) {
			...
			if (!OperatorChar.Contains(ch)) {
				...
				var start = i;
				while (i + 1 < expr.Length && !OperatorChar.Contains(expr[i + 1])) {
					i++;
				}
			...
			} else // operators
			{
				...
				if (op == "(" || OperatorArity[op] == 1) {
					if (i != 0
					    ...
					    && tokens.Count > 0
					    && !LocalFunctions.ContainsKey(tokens[^1]) // prev token is not function
					    && !LocalStringFunctions.ContainsKey(tokens[^1])
					   )
			...
			if (LocalFunctions.Keys.Contains(functionName)) {
				...
				if (exprTokens.Contains(",") && !functionName.Equals("len", StringComparison.OrdinalIgnoreCase)) {
					...
					for (var i = 0; i < exprTokens.Count; i++) {
						...
						while (i < firstCommaOrEndOfExpression)
							defaultExpr.Add(exprTokens[i++]);
						var argExpr = string.Join("", defaultExpr);
						sargs.Add(argExpr);
					}
					...
					// but if we only have one argument, then we pass it directly to the function
					var argExpr = string.Join("", exprTokens);
					tmpResult = LocalStringFunctions[functionName]([argExpr]);
				}
			} else {
	...
	private static void ApplyStringEqual(List<string> tokens, int opIndex) {
		...
		{
			var result = tokens[opIndex - 1] == tokens[opIndex + 1] ? "1" : "0";
	...
	private static void ApplyStringCoalescing(List<string> tokens, int opIndex, string[] originalTokens) {
		...
		{
			var isNumeric = double.TryParse(tokens[opIndex - 1], NumberStyles.Float, CultureInfo, out var numberA);
			if (isNumeric) {
				tokens.RemoveRange(opIndex, 2);
			} else {
				tokens.RemoveRange(opIndex - 1, 2);
			}
		}
	}
