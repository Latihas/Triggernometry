...
public class Polygon {
	...
	public static Polygon Parse(string rawExpr) {
		var vertices = rawExpr
			.Split(';')
			.Select(rawXY => rawXY.ParseArgs<float, float>())
	...
	public List<Polygon> Triangulate(double tolerance = 0.05) {
		...
		while (vertices.Count > 3) {
			...
			var minIdx = vertices
				.Select((_, i) => (idx: i, angle: GetAngle(vertices, i)))
	...
	public /*private*/ double GetAngle(List<Vector2> vertices, int idx) {
		var n = vertices.Count;
		var v1 = vertices[(idx - 1 + n) % n] - vertices[idx]; // 当前点 -> 前一个点
		var v2 = vertices[(idx + 1) % n] - vertices[idx]; // 当前点 -> 后一个点
	...
	public List<IsoscelesTriangle> IsoscelesTriangulate(double tolerance = 0.05) {
		...
		} else // 钝角
		{
			...
			var H = new Vector2(B.X + t * dx, B.Y + t * dy);
			// 按直角处理
			var Mb = (C + A) / 2;
			var Ma = (C + B) / 2;
