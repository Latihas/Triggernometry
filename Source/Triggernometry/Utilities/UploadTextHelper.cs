...
public class UploadTextHelper {
    private const string Base3500Chars =
    ...
    private const int Base = 3500;

    private const string url1 = "https://uploadtext-wxmxyorltv.cn-hangzhou.fcapp.run/"; // UploadText
    private const string url2 = "https://uploadtext-v-pzntyuecgf.cn-hangzhou.fcapp.run/"; // UploadText-v2

    /// <summary>
    ///     向阿里云函数计算上传文本的回调方法。<br />
    /// </summary>
    /// <param name="text">要上传的文本</param>
    ...
    public static string UploadText(string text, string url) {
        ...
        try {
            ...
            using (resp)
            using (var reader = new StreamReader(resp.GetResponseStream(), Encoding.UTF8)) {
                var serverErr = reader.ReadToEnd();
                throw new WebException($"网络或服务器错误：{serverErr}", ex);
            }
        }
    }

    public static string UploadTextV1(string text)
        => UploadText(text, url1);

    public static string UploadTextV2(string text)
        => UploadText(GetFakeName() + "\n" + text, url2);

    public static void UploadTextV1Callback(object _, string text) {
        try {
            _ = UploadTextV1(text);
        }
        catch (Exception ex) {
    ...
    public static void UploadTextV2Callback(object _, string text) {
        try {
            _ = UploadTextV2(text);
        }
        catch (Exception ex) {
    ...
    public static string GetFakeName() {
        ...
        if (Base3500Chars.Length != Base)
            throw new InvalidOperationException("Base3500Chars length is not 3500");

        var name = new char[4];

        // 取前 4 段字节组，每组 4 字节，取余映射到字符表
        for (var i = 0; i < 4; i++) {
            var value = BitConverter.ToUInt32(bytes, i * 4);
            var index = (int)(value % Base);
            name[i] = Base3500Chars[index];
        }
