...
public static class Memory {
	...
	private static void UpdateProcCacheIfNeeded() // any better ways to monitor the changing of ffxiv process?
	{
		if ((DateTime.Now - _lastUpdateTime).TotalSeconds > CACHE_INTERVAL_SECONDS) {
			...
			_xivProc = BridgeFFXIV.GetProcess();
	...
	internal static void OnXivProcUpdated() {
		GameLanguage.ScanOffsets();
		foreach (var action in _xivProcUpdatedActions.Values) {
			try {
				action.Invoke();
			} catch (Exception ex) {
				Instance.UnfilteredAddToLog(DebugLevelEnum.Error, ex.ToString());
			}
		}
	}
	...
	public static byte[] ReadModuleData(Process proc) {
		var moduleSize = proc.MainModule.ModuleMemorySize;
		var moduleData = new byte[moduleSize];
	...
	public static T Read<T>(IntPtr procHandle, IntPtr address) where T : struct {
		...
		if (!ReadProcessMemory(procHandle, address, valueBytes, (uint)size, out var bytesRead) || bytesRead != size) {
	...
	public static void Write<T>(IntPtr procHandle, IntPtr address, T newValue) where T : struct {
		var valueBytes = StructureToBytes(newValue);
		if (!WriteProcessMemory(procHandle, address, valueBytes, (uint)valueBytes.Length, out var bytesWritten) || bytesWritten != valueBytes.Length) {
	...
	public class ScanPattern {
		...
		public ScanPattern(string patternStr) {
			...
			var splittedRawPattern = RawPattern.Split([' '], StringSplitOptions.RemoveEmptyEntries);
			var regexStr = "";
			var bytes = new List<byte?>();
			for (var i = 0; i < splittedRawPattern.Length; i++) {
				switch (splittedRawPattern[i]) {
					...
					case "*":
						...
						Jump = true;
						regexStr += "(?<jump>.{4})";
						bytes.AddRange([null, null, null, null]);
						i += 3;
						break;
					default:
						if (byte.TryParse(splittedRawPattern[i], NumberStyles.HexNumber, null, out var result)) {
							regexStr += "\\x" + splittedRawPattern[i].PadLeft(2, '0');
							bytes.Add(result);
							break;
						}
						throw new Exception($"Invalid byte {splittedRawPattern[i]} in pattern {patternStr}");
				}
			}
	...
	public static int? ScanPoint(byte[] data, ScanPattern pattern, bool showWarning = true) {
		var dataStr = Encoding.GetEncoding("ISO-8859-1").GetString(data); // Latin-1
		int address;

		var match = pattern.PatternRegex.Match(dataStr);
		if (match.Success) {
		...
		if (pattern.Jump) {
			var offset = BytesToStructure<int>(data, address);
			address += offset + 4;
		}
		return address;
	}
	...
	public static IntPtr? ScanAddress(byte[] data, ScanPattern pattern) {
		var offset = ScanPoint(data, pattern);
	...
	public static Exception ScanNotFoundException(string varDescription)
		=> new(I18n.Translate("internal/Memory/scannotfoundexception",
	...
	private static readonly SemaphoreSlim _semaphoreSlim1B = new(1, 1);
	...
	public static async void UpdateOffset1B() {
		...
		try {
			_offset1B = await Task.Run(() => GetHeadMarkerOffset());
		} catch (Exception e) {
	...
	private static int GetHeadMarkerOffset() => 0; // SE was not using this offset since 7.0
	/*
	    try
	    {   // 6.x
	...
	public static IntPtr MarkingControllerPtr {
		get {
			if (_markingControllerPtr == IntPtr.Zero) {
				...
				var offset = ScanPoint(moduleData, "48 8d 0d * * * * 4c 8b 85") ?? throw ScanNotFoundException("MarkingControllerPtr");
	...
	public static uint? EntityIdByTargetMarker(string rawType) {
		...
		if (Enum.TryParse(rawType, true, out TargetMarkerEnum type))
			return EntityIdByTargetMarker(type);
		if (int.TryParse(rawType, out var id))
			return EntityIdByTargetMarker((TargetMarkerEnum)id);
		return null;
	}

	public static uint EntityIdByTargetMarker(TargetMarkerEnum type) => Read<uint>(XivProcHandle, TargetMarkersPtr + 8 * (int)type);

	public static TargetMarkerEnum TargetMarkerOnEntity(uint entityId) {
		var mem = ReadBytes(XivProcHandle, TargetMarkersPtr, 8 * 28);
		for (var i = 0; i < 28; i++) {
			var value = BytesToStructure<uint>(mem, 8 * i);
			if (value == entityId) {
	...
	public class Waymark {
		...
		public static Waymark Read(WaymarkEnum type) {
			var bytes = ReadBytes(XivProcHandle, WaymarksPtr + SIZE * (int)type, SIZE);
			var wm = new Waymark {
				Type = type,
	...
	public class Waymarks : IEnumerable<Waymark> {
		...
		public static Waymarks Read() => new() {
