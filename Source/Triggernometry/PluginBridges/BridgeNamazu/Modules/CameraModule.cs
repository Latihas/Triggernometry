...
public class CameraModule : ModuleBase {
	...
	public unsafe float AngleV {
		get => Camera->DirV;
		set => Camera->DirV = value;
	}

	public static unsafe float AngleH // 和游戏的角度（南 = 0）是反的，补偿 pi
	{
		get {
			var actualValue = Camera->DirH;
		...
		{
			const double errθ = Math.PI / 40;
			var oldθ = Camera->DirH;
			...
			Camera->DirH = (float)writeValue;
		}
	}

	public unsafe float GetParam(string param) {
		if (Offsets.TryGetValue(param, out var offset)) {
			SafeMemory.Read<float>((IntPtr)Camera + offset, out var f);
			return f;
		}
	...
	public unsafe void SetParam(string param, float newValue) {
