...
internal class ObsController : IDisposable {
	...
	private class AuthChallenge {
		...
		internal string secret {
			get {
				var secret_string = GenerateHash(password + salt);
	...
	private void HandleHelloOp(HelloOp helloData, string password) {
		...
		if (helloData?.authentication?.challenge != null) {
			if (password != null && password.Length > 0) {
				...
				RealPlugin.Instance.UnfilteredAddToLog(RealPlugin.DebugLevelEnum.Error, I18n.Translate("internal/Action/obsauthpassword",
	...
	private string SendRequest(string requestType) => SendRequest(requestType, NewMessageID(), null);

	private string SendRequest(string requestType, string requestId) => SendRequest(requestType, requestId, null);

	private string SendRequest(string requestType, object requestData) => SendRequest(requestType, NewMessageID(), requestData);
	...
	private string SendRequestBatch(object[] requests) => SendRequestBatch(requests, NewMessageID(), false, (int)RequestBatchExecutionType.SerialRealtime);
	...
	internal void RestartRecording() {
		...
		stopRequest[0] = new RequestOp {
			requestType = "StopRecord"
		};
		stopRequest[1] = new RequestOp {
			requestType = "Sleep",
			requestData = new {
				sleepMillis = 200
			}
		};
		stopRequest[2] = new RequestOp {
	...
	internal void SetCurrentScene(string sceneName) {
		SendRequest("SetCurrentProgramScene", new {
			sceneName
		});
	}
	...
	internal void ShowHideSource(string scenename, string sourcename, bool sceneItemEnabled) {
		...
		lock (lockobj) {
			respCallbacks.Add(messageId, resp => {
				...
				SendRequest("SetSceneItemEnabled", new {
					sceneName = scenename,
					sceneItemId,
					sceneItemEnabled
				});
		...
		SendRequest("GetSceneItemId", messageId, new {
			sceneName = scenename,
