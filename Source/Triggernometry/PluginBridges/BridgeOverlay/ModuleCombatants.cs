...
internal static class ModuleCombatants {
	...
	*/
	...
	/// <returns>OpEntity.NullEntity() if not found.</returns>
	internal static Entity InternalGetMyself() => InternalGetEntities().FirstOrDefault() ?? OpEntity.NullEntity();
	...
	public class OpEntity : Entity {
		...
		public override Job Job => Job.TryGetJob(_entity.Job /*numeric id*/, out var result) ? result : Job.GetJob(0);
