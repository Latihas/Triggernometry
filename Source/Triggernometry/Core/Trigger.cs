...
public class Trigger {
	...
	[XmlIgnore] public Folder Parent { get; set; }

	[XmlIgnore] public Repository Repo { get; set; } = null;

	[XmlIgnore] public bool ZoneBlocked { get; set; } = false;

	[XmlIgnore] private DateTime LastFired { get; set; }

	[XmlIgnore] internal DateTime RefireDelayedUntil { get; set; }

	[XmlIgnore] public Repository.RestrictionEnum RepoRestrictions { get; internal set; } = Repository.RestrictionEnum.None;

	[XmlIgnore] public string LogName => $"{Name} ({(Repo != null ? "@" : "")}{Id})";

	[XmlIgnore] public string FullPath => Parent == null ? Name : $@"{Parent.FullPath}\{Name}";
	...
	[XmlAttribute] public Guid Id { get; set; } = Guid.NewGuid();
	...
	/// </summary>
	[XmlIgnore] public TriggerSourceEnum Source { get; set; } = TriggerSourceEnum.Log;

	[XmlAttribute("Source")] public string Xml_Source {
	...
	/// </summary>
	[XmlIgnore] public PrevActionsEnum PrevActions { get; set; } = PrevActionsEnum.Keep;

	[XmlAttribute("PrevActions")] public string Xml_PrevActions {
	...
	/// </summary>
	[XmlIgnore] public RefireEnum PrevActionsRefire { get; set; } = RefireEnum.Allow;

	[XmlAttribute("PrevActionsRefire")] public string Xml_PrevActionsRefire {
	...
	/// </summary>
	[XmlIgnore] public SchedulingEnum Scheduling { get; set; } = SchedulingEnum.FromFire;

	[XmlAttribute("Scheduling")] public string Xml_Scheduling {
	...
	/// </summary>
	[XmlIgnore] public RefireEnum PeriodRefire { get; set; } = RefireEnum.Allow;

	[XmlAttribute("PeriodRefire")] public string Xml_PeriodRefire {
	...
	/// </summary>
	[XmlIgnore] public string RefirePeriodExpression { get; set; } = "0";
	...
	/// </summary>
	[XmlIgnore] public string MutexToCapture { get; set; }

	[XmlAttribute("MutexToCapture")] public string Xml_MutexToCapture {
	...
	/// </summary>
	[XmlIgnore] public bool EditAutofire { get; set; }

	[XmlAttribute("EditAutofire")] public string Xml_EditAutofire {
	...
	/// </summary>
	[XmlIgnore] public bool EditAutofireAllowCondition { get; set; }
	...
	/// </summary>
	[XmlIgnore] public bool Sequential { get; set; }

	[XmlAttribute("Sequential")] public string Xml_Sequential {
	...
	/// </summary>
	[XmlIgnore] public RealPlugin.DebugLevelEnum DebugLevel { get; set; } = RealPlugin.DebugLevelEnum.Inherit;
	...
	/// </summary>
	[XmlIgnore] public string TestInput { get; set; }

	[XmlAttribute("TestInput")] public string Xml_TestInput {
	...
	/// </summary>
	[XmlIgnore] public bool IsReadme { get; set; }

	[XmlAttribute("IsReadme")] public string Xml_IsReadme {
	...
	[Obsolete("Old XML alias, kept for backward compatibility.")] [XmlAttribute("AllowRefire")]
	public string Xml_AllowRefire {
		get => null;
		set => PrevActionsRefire = XmlAttr.Bool(value) ? RefireEnum.Allow : RefireEnum.Deny;
	}

	[Obsolete("Old XML alias, kept for backward compatibility.")] [XmlAttribute("RefireDelayed")]
	public string Xml_RefireDelayed {
		get => null;
		set => PeriodRefire = XmlAttr.Bool(value) ? RefireEnum.Deny : RefireEnum.Allow;
	}

	[Obsolete("Old XML alias, kept for backward compatibility.")] [XmlAttribute("RefireDelayExpression")]
	...
	internal void AddToLog(RealPlugin.DebugLevelEnum level, string message) {
		var configLevel = GetDebugLevel(RealPlugin.Instance);
	...
	///     · <see langword="true" /> if actions are queued, or a deferred fire is successfully scheduled (mutex capture).
	///     <br />
	...
	public bool Fire(Context ctx, RealPlugin.MutexInformation mtx = null) {
		try {
			...
			var prevLastFired = UpdateFireTiming(ctx);
			var scheduleBaseTime = ComputeScheduleBaseTime(ctx, prevLastFired);
	...
	///     · <see langword="false" /> if mutex capture is not applicable (mtx already present or no MutexToCapture
	///     configured).
	/// </summary>
	private bool TryScheduleDeferredFireWithMutexCapture(Context ctx, RealPlugin.MutexInformation mtx) {
		...
		var mn = ctx.EvaluateStringExpression(TriggerContextLogger, ctx.Plugin, MutexToCapture);
		var mi = ctx.Plugin.GetMutex(mn);
		var m = mi.QueueForAcquisition(ctx);
	...
	private DateTime ComputeScheduleBaseTime(Context ctx, DateTime previousLastFired) {
		var scheduleBaseTime = DateTime.Now;
	...
	private bool ApplyPrevActionsPolicy(Context ctx) {
		if (PrevActions == PrevActionsEnum.Interrupt) {
			var removedCount = RemoveSelfQueuedActions(ctx);
			if (removedCount > 0) {
		...
		if (PrevActionsRefire == RefireEnum.Deny) {
			var queuedCount = CountSelfQueuedActions(ctx);
			if (queuedCount > 0) {
				AddToLog(RealPlugin.DebugLevelEnum.Info, I18n.Translate(
	...
	private int RemoveSelfQueuedActions(Context ctx) {
		var removedCount = 0;
		lock (ctx.Plugin.ActionQueue) {
			foreach (var qa in GetSelfQueuedActions(ctx)) {
