...
public abstract class ActionBase {
    ...
    /// <summary>
    ///     When an action is queued for execution, it's wrapped in an ActionInstance that carries all the relevant data for
    ///     execution.
    /// </summary>
    ...
    public class ActionBundle {
        [XmlArrayItem("Action")] public List<ActionOld> Actions { get; set; } = [];
    ...
    [XmlAttribute("Enabled")] public string Xml_Enabled
    {
    ...
    [XmlIgnore] public Trigger ParentTrigger { get; set; }


    [XmlIgnore] public string ExecutionDelayExpression { get; set; } = "0";
    ...
    [XmlIgnore] public bool Asynchronous { get; set; } = true;

    [XmlAttribute("Asynchronous")] public string Xml_Asynchronous
    {
    ...
    [XmlIgnore] public DebugLevelEnum DebugLevel { get; set; } = DebugLevelEnum.Inherit;

    [XmlAttribute("DebugLevel")] public string Xml_DebugLevel
    {
    ...
    [XmlIgnore] public bool RefireInterrupt { get; set; }

    [XmlAttribute("RefireInterrupt")] public string Xml_RefireInterrupt
    {
    ...
    [XmlIgnore] public bool RefireRequeue { get; set; } = true;

    [XmlAttribute("RefireRequeue")] public string Xml_RefireRequeue
    {
    ...
    [XmlAttribute] public int OrderNumber;


    [XmlIgnore] public string Description { get; set; } = "";

    [XmlAttribute("Description")] public string Xml_Description
    {
    ...
    [XmlIgnore] public string DescBgColor { get; set; } = "";

    [XmlAttribute("DescBgColor")] public string Xml_DescBgColor
    {
    ...
    [XmlIgnore] public string DescTextColor { get; set; } = "";

    [XmlAttribute("DescTextColor")] public string Xml_DescTextColor
    {
    ...
    [XmlIgnore] public bool DescriptionOverride { get; set; }

    [XmlAttribute("DescriptionOverride")] public string Xml_DescriptionOverride
    {
    ...
    [XmlIgnore] public string Tag { get; set; }

    [XmlAttribute("Tag")] public string Xml_Tag
    {
    ...
    [XmlIgnore] public ConditionGroup Condition = new() {
        Enabled = false
    };

    [XmlElement("Condition")] public ConditionGroup Xml_Condition
    {
    ...
    public string Describe() {
        try {
            ...
            var temp = I18n.TrlAsync(Asynchronous);
            if (!string.IsNullOrWhiteSpace(ExecutionDelayExpression) && ExecutionDelayExpression.Trim() != "0") {
                var delay = double.TryParse(ExecutionDelayExpression.Trim(), NumberStyles.Float, CultureInfo.InvariantCulture, out _) ? ExecutionDelayExpression : $"({ExecutionDelayExpression})";
    ...
    /// <summary>
    ///     True = action was executed (enabled & conditions met), false = action was not executed (disabled or conditions not
    ///     met)
    /// </summary>
    ...
    protected IOrderedEnumerable<int> ApplySorting(int elementCount, List<bool> isNumeric, List<bool> isAscending, List<List<string>> values) {
        ...
        var indices = Enumerable.Range(0, elementCount);
        IOrderedEnumerable<int> sortedIndices = null;

        // Iterate through the sorting key functions
        for (var keyIndex = 0; keyIndex < values.Count; keyIndex++) {
            var k = keyIndex; // local variable for lambda expression
    ...
    protected static void CheckInvalidDymanicExpr(string expr, string[] invalidExprs) {
        foreach (var word in invalidExprs) {
    ...
    protected void ParseSortKeyFunctions(string rawExpr,
        ...
        keysExpr = []; // expression of the keys
        values = []; // each sublist contains the evaluated results of one key

        var regexSortKeyExpr = new Regex("^ *(?<type>[NnSs]) *(?<order>[-+]?) *:(?<key>.+)$");
        foreach (var rawKey in rawKeys) {
            var keyMatch = regexSortKeyExpr.Match(rawKey);
            if (keyMatch.Success) {
    ...
    private List<(PropertyInfo prop, ActionAttribute attr)> GetProperties() {
        ...
        var results = new List<(PropertyInfo prop, ActionAttribute attr)>();
        foreach (var pi in props) {
            var aa = pi.GetCustomAttributes<ActionAttribute>().FirstOrDefault();
            if (aa == null) {
                continue;
            }
    ...
    private void Cb_SelectedIndexChanged(object sender, EventArgs e) {
        ...
        var eb = (EnumBinding)cb.SelectedItem;
        var pi = eb.Prop;
    ...
    private void Clb_ItemCheck(object sender, ItemCheckEventArgs e) {
        ...
        for (var i = 0; i < clb.Items.Count; i++) {
            ...
            var eb = (EnumBinding)clb.Items[i];
            var thisval = (int)Enum.Parse(pi.PropertyType, eb.EnumValueName);
            newval |= thisval;
        }
    ...
    private Control GetGenericPropertyEditor() {
        ...
        var propeditors = new List<(PropertyInfo prop, ActionAttribute attr, object ctrl)>();
        foreach (var prop in props) {
            var temp = GetPropertyEditor(prop);
            if (temp != null) {
            ...
            if (prop.attr._typehint == typeof(string)) {
                switch (prop.attr._specialtype) {
                    ...
                    case ActionAttribute.SpecialTypeEnum.ImageSelector: {
                        ...
                        propeditors.Add((prop.prop, prop.attr, ctrl: new object[] {
                            etb, browseBtn
                        }));
                        break;
                    }
                ...
                if (prop.prop.PropertyType.IsDefined(typeof(FlagsAttribute), true)) {
                    ...
                    var names = Enum.GetNames(prop.prop.PropertyType);
                    var curval = (int)prop.prop.GetValue(this);
                    foreach (var name in names) {
                        // for example, "Internal/Enum/ActionTriggerOperation/ForceEnum/SkipConditions"
                        var thisval = (int)Enum.Parse(prop.prop.PropertyType, name);
                        if (thisval != 0 && (thisval & thisval - 1) == 0) {
                            // show only single bit values
                            var trkey = "Internal/Enum/" + prop.prop.PropertyType.DeclaringType.Name + "/" + prop.prop.PropertyType.Name + "/" + name;
                            var eb = new EnumBinding {
                                Text = trkey,
                                Prop = prop.prop,
                                EnumValueName = name
                            };
                    ...
                    var names = Enum.GetNames(prop.prop.PropertyType);
                    var curval = prop.prop.GetValue(this).ToString();
                    foreach (var name in names) {
        ...
        for (var i = 0; i < propeditors.Count; i++) {
            tlp.RowStyles.Add(new RowStyle(SizeType.AutoSize));
            var l = new Label();
            // for example, "Internal/Property/ActionActInteraction/Operation"
            var trkey = "Internal/Property/" + GetType().Name + "/" + Regex.Replace(propeditors[i].prop.Name, "[^a-zA-Z0-9]", "");
            l.Text = trkey;
    ...
    protected virtual Control GetPropertyEditor((PropertyInfo prop, ActionAttribute attr) prop) => null;

    /// <summary>
    ///     Creates a property editor control for the action. Actions can override this to provide their own entirely custom
    ///     property editor.
    ///     Alternatively, if controls want to use the generic property editor but provide a custom editor only for one
    ///     parameter, override the PropertyInfo overload instead.
    /// </summary>
