...
[XmlRoot(ElementName = "ObsControl")]
	...
	/// </summary>
	[XmlIgnore] [Action(1)] public OperationEnum Operation { get; set; } = OperationEnum.StartStreaming;
	...
	/// </summary>
	[XmlIgnore] [Action(2)] public string Endpoint { get; set; } = @"ws://${_const[OBSWebsocketEndpoint]}:${_const[OBSWebsocketPort]}";
	...
	/// </summary>
	[XmlIgnore] [Action(3)] public string Password { get; set; } = @"${_const[OBSWebsocketPassword]}";

	[XmlAttribute("Password")] public string Xml_Password {
	...
	/// </summary>
	[XmlIgnore] [Action(4)] public string SceneName { get; set; } = "";

	[XmlAttribute("SceneName")] public string Xml_SceneName {
	...
	/// </summary>
	[XmlIgnore] [Action(5)] public string SourceName { get; set; } = "";

	[XmlAttribute("SourceName")] public string Xml_SourceName {
	...
	///     Custom JSON payload
	/// </summary>
	[XmlIgnore] [Action(6)] public string JSONPayload { get; set; } = "";

	[XmlAttribute("JSONPayload")] public string Xml_JSONPayload {
			...
			try {
				switch (Operation) {
					...
					case OperationEnum.SaveReplayBuffer:
						...
						break;
					case OperationEnum.SetScene: {
						var scn = ctx.EvaluateStringExpression(ActionContextLogger, ctx, SceneName);
						obsController.SetCurrentScene(scn);
					}
						break;
					case OperationEnum.ShowSource: {
						var scn = ctx.EvaluateStringExpression(ActionContextLogger, ctx, SceneName);
						var src = ctx.EvaluateStringExpression(ActionContextLogger, ctx, SourceName);
					...
					case OperationEnum.HideSource: {
						...
						break;
					case OperationEnum.JSONPayload: {
						var json = ctx.EvaluateStringExpression(ActionContextLogger, ctx, JSONPayload);
