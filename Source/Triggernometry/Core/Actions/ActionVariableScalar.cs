...
public class ActionVariableScalar : ActionBase {
	...
	/// </summary>
	[XmlIgnore] [Action(3)] public string JsonTargetName { get; set; } = "";

	[XmlAttribute("JsonTargetName")] public string Xml_JsonTargetName {
	...
	///     Value expression
	/// </summary>
	[XmlIgnore] [Action(4)] public string Value { get; set; } = "";
	...
	public bool JsonTargetPersistent { get; set; }

	[XmlAttribute("JsonTargetPersistent")] public string Xml_JsonTargetPersistent {
	...
	/// </summary>
	[XmlIgnore] [Action(6)] // todo need to couple this with variable on editor
	...
	internal override string DescribeImplementation() {
		...
		var tPersist = I18n.TrlVarPersist(JsonTargetPersistent);
		switch (Operation) {
			case OperationEnum.SetNumeric:
			case OperationEnum.SetString:
				var exprType = I18n.TrlExprType(Operation == OperationEnum.SetString);
			...
			case OperationEnum.Increment:
				var value = string.IsNullOrWhiteSpace(Value) ? "1" : Value;
				return I18n.Translate(
	...
	internal override void ExecuteImplementation(ActionInstance ai) {
		...
		var varname = ctx.EvaluateStringExpression(ActionContextLogger, ctx, Name);
		var sPersist = I18n.TrlVarPersist(Persistent);
		var tPersist = I18n.TrlVarPersist(JsonTargetPersistent);
		string changer;
		if (ctx.Trigger != null) {
		...
		switch (Operation) {
			...
			case OperationEnum.Increment: {
				double original = 0;
				var increment = string.IsNullOrWhiteSpace(Value)
					? 1
					: ctx.EvaluateNumericExpression(ActionContextLogger, ctx, Value);
				var x = new VariableScalar {
					LastChanger = changer,
					LastChanged = DateTime.Now
				};
				lock (vs.Scalar) {
					if (vs.Scalar.TryGetValue(varname, out var originalVar)) {
			...
			case OperationEnum.QueryJsonPath: {
				...
				var tgtname = ctx.EvaluateStringExpression(ActionContextLogger, ctx, JsonTargetName);
				var vs2 = plug.GetVariableStore(JsonTargetPersistent);
				var query = ctx.EvaluateStringExpression(ActionContextLogger, ctx, Value);
				var pc = new JsonPathContext();
