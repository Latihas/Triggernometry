...
internal class ActionKeypress : ActionBase {
    ...
    /// </summary>
    [XmlIgnore] [Action(1)] public OperationEnum Operation { get; set; } = OperationEnum.SendKeys;
    ...
    /// </summary>
    [XmlIgnore] [Action(2, specialtype: ActionAttribute.SpecialTypeEnum.KeypressRecorder)]
    public string Keypress { get; set; } = "";

    [XmlAttribute("Keypress")] public string Xml_Keypress
    {
    ...
    /// </summary>
    [XmlIgnore] [Action(3, specialtype: ActionAttribute.SpecialTypeEnum.KeypressRecorder)]
    public string Keycode { get; set; } = "";

    [XmlAttribute("Keycode")] public string Xml_Keycode
    {
    ...
    /// </summary>
    [XmlIgnore] [Action(4)] public string WindowTitle { get; set; } = "";

    [XmlAttribute("WindowTitle")] public string Xml_WindowTitle
    {
    ...
    /// </summary>
    [XmlIgnore] [Action(5, typeof(int))] public string ProcessId { get; set; } = "";

    [XmlAttribute("ProcessId")] public string Xml_ProcessId
    {
    ...
    internal override string DescribeImplementation() {
        switch (Operation) {
            ...
            case OperationEnum.WindowMessageCombo:
                ...
                var title = string.IsNullOrWhiteSpace(WindowTitle) ? ".*" : WindowTitle;
                var parsedProcId = 1;
                try {
                    parsedProcId = (int)MathParser.Parse(ProcessId);
                }
                catch {
                }
                if (parsedProcId == 0) {
    ...
    internal override void ExecuteImplementation(ActionInstance ai) {
        ...
        switch (Operation) {
            case OperationEnum.SendKeys: {
                var ks = ctx.EvaluateStringExpression(ActionContextLogger, ctx, Keypress);
                SendKeys.SendWait(ks);
            }
                break;
            case OperationEnum.WindowMessage: {
                var procid = (int)ctx.EvaluateNumericExpression(ActionContextLogger, ctx, ProcessId);
                var window = ctx.EvaluateStringExpression(ActionContextLogger, ctx, WindowTitle);
                var keycode = (int)ctx.EvaluateNumericExpression(ActionContextLogger, ctx, Keycode);
            ...
            case OperationEnum.WindowMessageCombo: {
                var procid = (int)ctx.EvaluateNumericExpression(ActionContextLogger, ctx, ProcessId);
                var window = ctx.EvaluateStringExpression(ActionContextLogger, ctx, WindowTitle);
                var keycodes = ctx.EvaluateStringExpression(ActionContextLogger, ctx, Keycode)
