...
internal class ActionBeep : ActionBase {
    ...
    /// </summary>
    [XmlIgnore] [Action(1, typeof(float))] public string Frequency { get; set; } = "1046.5"; // freq(C6)

    [XmlAttribute("Frequency")] public string Xml_Frequency
    {
    ...
    /// </summary>
    [XmlIgnore] [Action(2, typeof(int))] public string Duration { get; set; } = "100";

    [XmlAttribute("Duration")] public string Xml_Duration
    {
    ...
    internal override void ExecuteImplementation(ActionInstance ai) {
        ...
        var beepLength = ctx.EvaluateNumericExpression(ActionContextLogger, ctx, Duration);
        if (beepLength < 0.0) {
            beepLength = 0.0;
        ...
        var frequency = ctx.EvaluateNumericExpression(ActionContextLogger, ctx, Frequency);
        if (!useSimulatedBeep) {
            if (frequency < 37.0) {
                frequency = 37.0;
        ...
        var volume = plug.cfg.SimulatedBeepVolume / 100.0;
