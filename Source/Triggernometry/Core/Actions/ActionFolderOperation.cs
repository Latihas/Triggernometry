...
[XmlRoot(ElementName = "FolderOperation")]
	...
	/// </summary>
	[XmlIgnore] [Action(1)] public OperationEnum Operation { get; set; } = OperationEnum.Enable;
	...
	/// </summary>
	[XmlIgnore] [Action(2, specialtype: ActionAttribute.SpecialTypeEnum.FolderReference)]
	public Guid FolderId { get; set; } = Guid.Empty;

	[XmlAttribute("FolderId")] public string Xml_FolderId {
	...
	internal override string DescribeImplementation() {
		var f = RealPlugin.Instance.GetFolderById(FolderId, ParentTrigger?.Repo);
		if (f != null) {
			switch (Operation) {
	...
	internal override void ExecuteImplementation(ActionInstance ai) {
		...
		var f = plug.GetFolderById(FolderId, ctx.Trigger?.Repo);
		if (f == null) {
		...
		switch (Operation) {
			...
			case OperationEnum.CancelTriggers: {
				...
				var removed = RealPlugin.Instance.CancelQueuedActions(_qa => _qa?.ctx?.Trigger != null && triggersInFolder.Contains(_qa.ctx.Trigger)
				);
