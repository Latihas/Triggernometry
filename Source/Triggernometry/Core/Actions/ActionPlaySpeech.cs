...
public class ActionPlaySpeech : ActionBase {
	...
	/// </summary>
	[XmlIgnore] [Action(1)] public Configuration.AudioRoutingMethodEnum AudioTarget { get; set; } = Configuration.AudioRoutingMethodEnum.None;

	[XmlAttribute("AudioTarget")] public string Xml_AudioTarget {
	...
	/// </summary>
	[XmlIgnore] [Action(3, typeof(float))] public string Volume { get; set; } = "100";

	[XmlAttribute("Volume")] public string Xml_Volume {
	...
	/// </summary>
	[XmlIgnore] [Action(4, typeof(int))] public string Rate { get; set; } = "0";

	[XmlAttribute("Rate")] public string Xml_Rate {
		get => XmlAttr.String(Rate, "0");
		set => Rate = value;
	}

	// todo remove?
	[XmlIgnore] [Action(5)] public bool ExclusivePlayer { get; set; } = true;

	[XmlAttribute("ExclusivePlayer")] public string Xml_ExclusivePlayer {
	...
	internal override string DescribeImplementation() => I18n.Translate("internal/Action/desctts", "say ({0}) at volume ({1}) %, using speed ({2})", Message, Volume, Rate);
