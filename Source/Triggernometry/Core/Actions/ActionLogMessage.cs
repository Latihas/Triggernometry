...
public class ActionLogMessage : ActionBase {
	...
	/// </summary>
	[XmlIgnore] [Action(1)] public LogEvent.SourceEnum Target { get; set; } = LogEvent.SourceEnum.Log;
	...
	/// </summary>
	[XmlIgnore] [Action(3)] public bool ProcessAsLogline { get; set; }

	[XmlAttribute("ProcessAsLogline")] public string Xml_ProcessAsLogline {
	...
	/// </summary>
	[XmlIgnore] [Action(4)] public bool AddToACTEncounter { get; set; }

	[XmlAttribute("AddToACTEncounter")] public string Xml_AddToACTEncounter {
	...
	/// </summary>
	[XmlIgnore] [Action(5)] public LogLevelEnum Level { get; set; } = LogLevelEnum.Error;

	[XmlAttribute("Level")] public string Xml_Level {
	...
	internal override void ExecuteImplementation(ActionInstance ai) {
		...
		var message = ctx.EvaluateStringExpression(ActionContextLogger, ctx, Message);

		if (ProcessAsLogline) {
			var zone = ctx.EvaluateStringExpression(ActionContextLogger, ctx, plug.currentZone);
			plug.LogLineQueuer(message, zone, Target);
		} else {
			var debugLevel = RealPlugin.DebugLevelEnum.Error;
			switch (Level) {
