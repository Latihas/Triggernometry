...
public class ActionVariableList : ActionBase {
	...
	///     Value expression
	/// </summary>
	[XmlIgnore] [Action(4)] public string ValueExpression { get; set; } = "";

	[XmlAttribute("ValueExpression")] public string Xml_ValueExpression {
	...
	/// </summary>
	[XmlIgnore] [Action(5)] public string Index { get; set; } = "";

	[XmlAttribute("Index")] public string Xml_Index {
	...
	/// </summary>
	[XmlIgnore] [Action(6)] public string Target { get; set; } = "";

	[XmlAttribute("Target")] public string Xml_Target {
	...
	/// </summary>
	[XmlIgnore] [Action(7)] // todo need to couple this with variable on editor
	...
	internal override string DescribeImplementation() {
		var sPersistL = I18n.TrlVarPersist(Persistent);
		var tPersistL = I18n.TrlVarPersist(TargetPersistent);
		var exprTypeL = I18n.TrlExprType(ValueType == ExpressionTypeEnum.String);
		switch (Operation) {
			...
			case OperationEnum.PopFirst: // the action was updated to "Pop" but the name was unchanged
				var index = string.IsNullOrWhiteSpace(Index) ? "1" : Index;
				return I18n.Translate(
			...
			case OperationEnum.SortAlphaDesc:
				var strOrder = I18n.TrlSortAscOrDesc(Operation == OperationEnum.SortAlphaAsc);
				return I18n.Translate(
			...
			case OperationEnum.SortNumericDesc:
				var numOrder = I18n.TrlSortAscOrDesc(Operation == OperationEnum.SortNumericAsc);
				return I18n.Translate(
			...
			case OperationEnum.SortFfxivPartyDesc:
				var jobOrder = I18n.TrlSortAscOrDesc(Operation == OperationEnum.SortFfxivPartyAsc);
				return I18n.Translate(
	...
	internal override void ExecuteImplementation(ActionInstance ai) {
		...
		switch (Operation) {
			case OperationEnum.Unset: {
				...
				break;
			case OperationEnum.Push: {
				var value = GetListExpressionValue(ctx, ValueType, ValueExpression);
				lock (svs.List) {
					var vl = svs.GetListVariable(sourcename, true);
					vl.Push(new VariableScalar {
						Value = value
					}, changer);
				}
			...
			case OperationEnum.Insert: {
				var value = GetListExpressionValue(ctx, ValueType, ValueExpression);
				var index = (int)ctx.EvaluateNumericExpression(ActionContextLogger, ctx, Index);
				lock (svs.List) {
					var vl = svs.GetListVariable(sourcename, true);
			...
			case OperationEnum.SetAll: {
				...
				CheckInvalidDymanicExpr(ValueExpression, invalidExprs);
				var newLength = (int)ctx.EvaluateNumericExpression(ActionContextLogger, ctx, Index);
				// create a new list to avoid setting values base on previously edited ones
				var vlNew = new VariableList();
				lock (svs.List) {
					var vl = svs.GetListVariable(sourcename, false);
					newLength = newLength <= 0 ? vl.Size : newLength;
					ctx.varName = (Persistent ? "plvar:" : "lvar:") + sourcename; // ${_this}
					for (var i = 1; i <= newLength; i++) // index starts from 1
					{
						ctx.listIndex = i; // ${_idx}
						var expr = GetListExpressionValue(ctx, ValueType, ValueExpression);
						vlNew.Push(new VariableScalar {
							Value = expr
						}, changer);
					}
			...
			case OperationEnum.PopFirst: {
				var targetname = ctx.EvaluateStringExpression(ActionContextLogger, ctx, Target);
				var rawIndex = string.IsNullOrWhiteSpace(Index) ? 1 : (int)ctx.EvaluateNumericExpression(ActionContextLogger, ctx, Index);
			...
			case OperationEnum.PopToListSet: {
				var targetname = ctx.EvaluateStringExpression(ActionContextLogger, ctx, Target);
				var isInsert = Operation == OperationEnum.PopToListInsert;
				var popToEnd = isInsert && string.IsNullOrWhiteSpace(ValueExpression);

				var rawSourceIndex = (int)ctx.EvaluateNumericExpression(ActionContextLogger, ctx, Index);
				var rawTargetIndex = 0;
				if (!popToEnd) {
				...
				lock (svs.List) {
					var svl = svs.GetListVariable(sourcename, false);
				...
				lock (tvs.List) {
					var tvl = tvs.GetListVariable(targetname, true);
					if (popToEnd) {
				...
				break;
			case OperationEnum.SortAlphaAsc: {
				var order = I18n.TrlSortAscOrDesc(true);
				lock (svs.List) {
					var vl = svs.GetListVariable(sourcename, false);
				...
				break;
			case OperationEnum.SortAlphaDesc: {
				var order = I18n.TrlSortAscOrDesc(false);
				lock (svs.List) {
					var vl = svs.GetListVariable(sourcename, false);
			...
			case OperationEnum.SortByKeys: {
				...
				ParseSortKeyFunctions(ValueExpression,
					out var isNumeric, out var isAscending,
				...
				lock (svs.List) {
					...
					for (var listIndex = 0; listIndex < vl.Size; listIndex++) {
					...
					var sortedIndices = ApplySorting(vl.Size, isNumeric, isAscending, values);
			...
			case OperationEnum.Copy: {
				...
				lock (tvs.List) {
					var newvl = new VariableList();
					foreach (var x in vl.Values) {
			...
			case OperationEnum.InsertList: {
				var targetname = ctx.EvaluateStringExpression(ActionContextLogger, ctx, Target);
				var rawIndex = (int)ctx.EvaluateNumericExpression(ActionContextLogger, ctx, Index);
				VariableList vl = null;
				VariableList vlcopy = null;
				lock (svs.List) {
					vl = svs.GetListVariable(sourcename, false);
					if (svs == tvs) {
						var newvl = tvs.GetListVariable(targetname, true);
			...
			case OperationEnum.Filter: {
				...
				lock (svs.List) {
					var vl = svs.GetListVariable(sourcename, false);
					ctx.varName = (Persistent ? "plvar:" : "lvar:") + Name; // for ${_this}
					for (var i = 0; i < vl.Size; i++) {
						ctx.listIndex = i + 1; // for ${_idx}
						var result = ctx.EvaluateNumericExpression(ActionContextLogger, ctx, ValueExpression);
				...
				break;
			case OperationEnum.Join: {
				var separator = GetListExpressionValue(ctx, ValueType, ValueExpression);
				var targetname = ctx.EvaluateStringExpression(ActionContextLogger, ctx, Target);
				var newval = "";
				lock (svs.List) {
					var vl = svs.GetListVariable(sourcename, false);
			...
			case OperationEnum.Split: {
				...
				{
					if (svs.Scalar.TryGetValue(sourcename, out var value)) {
						splitval = value.Value;
					}
				}
				var vals = splitval.Split([separator], StringSplitOptions.None);
