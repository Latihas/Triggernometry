...
public class Repository {
	...
	/// </summary>
	[XmlAttribute] public DateTime LocalLastModified { get; set; } = DateTime.MinValue;

	/// <summary>The time of the most recent update check, no matter updated or not.</summary>
	[XmlAttribute] public DateTime UpdateLastChecked { get; set; } = DateTime.MinValue;
	...
	/// </summary>
	[XmlAttribute] public int UpdateInterval { get; set; } = 5;

	[XmlAttribute] public bool AllowScriptExecution { get; set; }
	[XmlAttribute] public bool AllowProcessLaunch { get; set; }
	[XmlAttribute] public bool AllowWindowMessages { get; set; }
	[XmlAttribute] public bool AllowObsControl { get; set; }
	[XmlAttribute] public bool AllowDiskOperations { get; set; }

	/// <summary>Determines how newly imported folders or triggers should be enabled.</summary>
	[XmlAttribute] public NewBehaviorEnum NewBehavior { get; set; }

	/// <summary>Determines when remote update checks should be performed.</summary>
	[XmlAttribute] public UpdatePolicyEnum UpdatePolicy { get; set; }

	/// <summary>Determines whether repository settings override the audio output method.</summary>
	[XmlAttribute] public AudioOutputEnum AudioOutput { get; set; }
	...
	public void ClearContent() {
		Instance.RemoveTriggersFromFolder(Root);
		Root.Folders.Clear();
		Root.Triggers.Clear();
	...
	/// <summary>
	///     Returns all <see cref="RestrictionEnum" /> flags violated by the actions and stores them in
	///     <see cref="Trigger.RepoRestrictions" />.
	/// </summary>
	...
	internal RestrictionEnum ApplySettingsOnFolder(Folder f) {
		...
		if (savedFolder == null) // new
		{
			...
			FolderStates.Add(new RepositoryItem {
				Id = f.Id,
				Enabled = f.Enabled
			});
		} else // saved
	...
	internal RestrictionEnum ApplySettingsOnTrigger(Trigger t) {
		...
		if (savedTrigger == null) // new
		{
			...
			TriggerStates.Add(new RepositoryItem {
				Id = t.Id,
				Enabled = t.Enabled
			});
		} else // saved
	...
	/// <summary>
	///     Imports exported folders and triggers from
	///     <see cref="TriggernometryExport"> into this repository and applies all repo settings.
	/// </summary>
	...
	private bool ShouldUpdate(long remoteLength, DateTime remoteLastModified) {
		...
		DateTime cacheLimit;
		try {
			cacheLimit = LocalLastModified.AddMinutes(Instance.cfg.CacheRepoExpiry);
		} catch {
			cacheLimit = DateTime.MinValue;
		}
		var cacheExpired = cacheLimit < DateTime.Now;
		...
		var shouldUpdate = remoteLength != CurrentContentLength || remoteLastModified != LocalLastModified || cacheExpired;
	...
	private async Task<bool> UpdateAsync() {
		try {
			...
			var raw = await HttpHelper.GetBytesAsync(Address);

			// Encode and Deserialize
			var data = Encoding.UTF8.GetString(raw);
			var exp = TriggernometryExport.Unserialize(data);
			if (exp.Corrupted) {
	...
	public async Task CheckAndUpdateAsync(bool isStartup = false) {
		...
		var shouldCheckUpdate = !isStartup || UpdatePolicy == UpdatePolicyEnum.Startup;
	...
	internal static string BackupFolderPath => Path.Combine(Instance.ConfigPath, "TriggernometryRepoBackups");
